...
// Use 127.0.0.1 to avoid IPv6 (::1) vs IPv4 resolution issues on CI/macOS
...
test('mode picker flow and top nav appear', async ({ page }) => {
  ...
  // Clear onboarding flag so the modal doesn't block interaction
  await page.evaluate(() => {
    localStorage.removeItem('inspire:onboardingComplete');
  });
  await page.reload();
...
test('generate pack -> open card -> focus mode overlay', async ({ page }) => {
  ...
  await page.click('.mode-card >> nth=0');
  // select submode
  await page.click('.submode-card >> nth=0');
